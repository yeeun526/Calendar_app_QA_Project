...
-- 2. **중요도별** 결함 밀도 측정 <<마지막에 세미콜론 꼭!!>>
SELECT TC_ID, Title -- 3. where에서 선택된 행의 TC_ID와 title 보여줌
FROM test_results -- 1. test_result라는 엑셀 시트 가져옴
WHERE Status = 'Bug Found' AND Priority = 'P1'; -- 2. Bug Found에다가 P1(Urgent)인 행만 남김

-- 3. 전체 **합격률(Pass rate)** 확인
SELECT 
(COUNT(CASE WHEN Status = 'Done' THEN 1 END)*100.0/COUNT(*)) AS Pass_rate -- 1. case when == if
FROM test_results;